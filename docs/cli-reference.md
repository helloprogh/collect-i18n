...
确定性阶段还包含四条有界的内容展开通路：R3 滚屏（窗口与页面内滚动容器步进，至多 12 步并带稳定性早退）、R7 控件扫描（树展开图标、分页「下一页」按钮、以及逐轮打开的级联/下拉/日期选择面板等纯客户端控件，每轮至多 6 次点击、每路由至多 8 轮，不触碰任何表单或动作按钮）、R8 通用交互扫描（先点击未展开的 [role=tab]，再点击原生 button/[role=button]，每轮 1 个目标、至多 10 轮，点击后等待瞬态内容挂载、批量截图、Escape 收尾；全部基于 ARIA 语义，不依赖任何组件库选择器）与 R9 运行时证据镜像（插桩在每个 t() 值流经时——包括画布 formatter、瞬态 toast、命令式弹窗——把「键 → 渲染文本」追加到离屏镜像条；扫描结束后把镜像条临时拉回视口，对仍未取证但有镜像记录的键逐条截图为 B 级证据，随后恢复隐藏）。动态拼接来源的 needs_manual 键一旦在 DOM 或镜像中真实实例化即自动转正为 captured；死键永不实例化，留在人工队列。
...
**Agent 队列自动驱动**：确定性队列排空后，`run` 自动为剩余 Agent 任务生成 router-hint 配方计划（goto → waitForKey → capture），批次执行直至队列收敛、路由饱和或会话截止时间（至多 120 个计划）；已验证计划按 key_path 跨会话继承，重复跑批零重新推导。
...
确定性控件扫描（R7）依赖组件库 DOM 结构提示来展开折叠树、翻到下一页并打开级联/下拉/日期面板。内置默认值面向 Element Plus（`.el-tree-node__expand-icon:not(.is-leaf)`、`.el-pagination .btn-next`、`.el-cascader,.el-select,.el-select__wrapper,.el-date-editor,.el-sub-menu__title` 及 popper 关闭集合；侧边栏折叠子菜单展开为纯客户端状态，点击只展开不导航）；`browser.sweep` 可逐项覆盖（`treeExpandSelector` / `paginationNextSelector` / `deepWidgetSelector` / `closePopperSelector`）或整体关闭（`enabled: false`）。这样深度扫描对任一套组件库都可通过配置适配，引擎本身不再硬绑任何库。

交互定位层（下拉选项解析、歧义点击的对话框限定、exact-text 兜底的 toast/通知宿主）内置 Element Plus + ARIA 角色 + Ant Design/naive-ui/Arco 类名兜底；`browser.controls` 可按组追加项目自定义选择器（`dropdownOptionSelector` / `dialogSelector` / `toastHostSelector`，逗号分隔多个），追加语义与 `browser.loadingSelectors` 一致——内置选择器永不回退，任意组件库无需修改引擎即可适配。
