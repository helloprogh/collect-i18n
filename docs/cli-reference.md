...
确定性控件扫描（R7）依赖组件库 DOM 结构提示来展开折叠树、翻到下一页并打开级联/下拉/日期面板。内置默认值面向 Element Plus（`.el-tree-node__expand-icon:not(.is-leaf)`、`.el-pagination .btn-next`、`.el-cascader,.el-select,.el-select__wrapper,.el-date-editor` 及 popper 关闭集合）；`browser.sweep` 可逐项覆盖（`treeExpandSelector` / `paginationNextSelector` / `deepWidgetSelector` / `closePopperSelector`）或整体关闭（`enabled: false`）。这样深度扫描对任一套组件库都可通过配置适配，引擎本身不再硬绑任何库。
