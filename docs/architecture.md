...
确定性队列对每个路由的一次访问按固定顺序走五个通道，前一个通道留下未处理 key 时才进入下一个：

1. **批量解析（R2）**：对当前已挂载的候选 key 一次解析 + 逐个截图；
2. **滚动扫描（R3）**：至多 12 步下滚（窗口 + 页内滚动容器），让懒渲染/虚拟行挂载后再批量捕获；
3. **控件扫描（R7）**：展开折叠树、翻页、打开级联/下拉/日期面板与折叠侧边栏子菜单（选择器可经 `browser.sweep` 配置）；
4. **交互扫描（R8）**：每轮点击一个 ARIA tab 或原生按钮，批量捕获弹出的瞬态内容后 Escape 收尾；
5. **证据镜像（R9）**：插桩层把每个流经 `t()` 的值记录在离屏镜像条上，画布文本、瞬态 toast、命令式弹窗即使不在 DOM 里也能取得 B 级证据。

三条扫描通道共享同一条「过滤可见池 → 批量解析截图 → 记录结果」的尾部实现，只在天数驱动方式与空转退出条件上不同。B 级证据在隔离探针页执行一次性 canary 因果验证（多 key 合并为一次导航），通过后提升为 A 级。确定性队列的每类等待都有界，且 `run` 的确定性窗口超时后由 `run` 重新触发队列（`/api/deterministic/resume`），`run` 在仍有剩余工作时保持会话 running，Skill 按协议继续轮询或进入 Agent/人工阶段；只有 `complete` / `failed` / `restart` 才关闭会话。
