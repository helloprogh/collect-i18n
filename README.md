...
`skill/collect-i18n` 是遵循开放 Agent Skills 格式的通用 Skill，不依赖 Claude Code、Codex 或其他单一客户端的私有元数据。可将同一个发布包安装到客户端支持的 Skill 目录；例如跨客户端目录 `.agents/skills/`、Claude Code 的 `.claude/skills/` 或 Codex 的 `.codex/skills/`。

Skill 是 Agent 的操作层，CLI 和本地服务才是执行与事实层。Agent 只读取 `agent next` 返回的源码证据并生成受限的 TriggerPlan；路由、点击、填写、等待和请求 Mock 都由 CLI 在同一个真实浏览器中执行。
