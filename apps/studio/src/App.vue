    ...
    <main class="content" v-loading="loading">
      ...
      <template v-else>
        <section class="excel-grid">
          <article class="panel delivery-card"><span class="big-icon"><Download /></span><h2>导出翻译任务</h2><p>生成干净的 Excel，只有“中文、英文、截图、Key Path”四列。英文默认复制中文；尚未采集的词条保留空截图单元格。</p><ul><li>共 {{ status?.counts.total ?? 0 }} 个词条</li><li>已嵌入 {{ status?.capturedKeyCount ?? status?.counts.captured ?? 0 }} 个词条的截图</li><li v-if="status?.historicalEvidenceCount">已保留 {{ status.historicalEvidenceCount }} 份不同界面状态的历史证据，Excel 只使用每个词条的最新证据</li><li v-if="status?.duplicateHashCount">检测到 {{ status.duplicateHashCount }} 份重复哈希证据，请运行数据库维护</li><li>不包含状态列或隐藏工作表</li></ul><el-button type="primary" size="large" :icon="Download" @click="downloadWorkbook">下载四列 Excel</el-button></article>
